function [A,B] = calcVelMatricesFast(surface1,surface2,dval)
if nargin == 2
    dval = pi;
end
...
%    isself = false;
%end
% On-panel influence no longer requires points to come from an input surface
...
%end
% Arbitrary points can now be specified on panels
dtheta((abs(yp) < 1e-12) & (xp > 0) & (xp < x2)) = dval;
